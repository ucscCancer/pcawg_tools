...
def radia(chrom, args,
    ...
    if (rnaNormalFilename != None and rnaTumorFilename != None):
        cmd = "python radia.pyc %s %s -n %s -x % -t %s -r %s --dnaNormalFasta %s --rnaNormalFasta %s --dnaTumorFasta %s --rnaTumorFasta %s %s" %(args.patientId, chrom, 
                dnaNormalFilename, rnaNormalFilename, dnaTumorFilename, rnaTumorFilename, 
                dnaNormalFastaFilename, rnaNormalFastaFilename, dnaTumorFastaFilename, rnaTumorFastaFilename, args)
    # triplets
    elif (rnaTumorFilename != None):
        cmd = "python radia.pyc %s %s -n %s -t %s -r %s --dnaNormalFasta %s --dnaTumorFasta %s --rnaTumorFasta %s %s" %(args.patientId, chrom, 
                dnaNormalFilename, dnaTumorFilename, rnaTumorFilename,
                dnaNormalFastaFilename, dnaTumorFastaFilename, rnaTumorFastaFilename, args)
    # pairs
    else:
        cmd = "python radia.pyc %s %s -n %s -t %s -r %s --dnaNormalFasta %s --dnaTumorFasta %s %s" %(args.patientId, chrom, 
                dnaNormalFilename, dnaTumorFilename,
                dnaNormalFastaFilename, dnaTumorFastaFilename, args)
    
    if args.dnaNormalUseChr:
        cmd += ' --dnaNormalUseChr '
    if args.rnaNormalUseChr:
        cmd += ' --rnaNormalUseChr '
    if args.dnaTumorUseChr:
        cmd += ' --dnaTumorUseChr '
    if args.rnaTumorUseChr:
        cmd += ' --rnaTumorUseChr '
    if args.gzip:
        cmd += ' --gzip '
...
def radiaFilter(args, chrom, rawFile, outputDir, scriptsDir, blatFastaFilename):
    ...
    cmd = "python filterRadia.pyc %s %s %s %s %s --blatFastaFilename %s %s" %(args.patientId, chrom, rawFile, outputDir, scriptsDir, blatFastaFilename, args) 
...
def __main__():
    ...
    parser.add_argument("", "--canonical", action="store_true", default=False, dest="canonical", metavar="CANONICAL", help="include this argument if only the canonical transcripts from snpEff should be used, %default by default")
    ...
    parser.add_argument("", "--noBlacklist", action="store_false", default=True, dest="blacklist", help="include this argument if the blacklist filter should not be applied")
    parser.add_argument("", "--noTargets", action="store_false", default=True, dest="targets", help="include this argument if the target filter should not be applied")
    parser.add_argument("", "--noDbSnp", action="store_false", default=True, dest="dbSnp", help="include this argument if the dbSNP info/filter should not be applied")
    parser.add_argument("", "--noRetroGenes", action="store_false", default=True, dest="retroGenes", help="include this argument if the info/retrogenes filter should not be applied")
    parser.add_argument("", "--noPseudoGenes", action="store_false", default=True, dest="pseudoGenes", help="include this argument if the info/pseudogenes filter should not be applied")
    parser.add_argument("", "--noCosmic", action="store_false", default=True, dest="cosmic", help="include this argument if the cosmic annotation should not be applied")
    parser.add_argument("", "--noBlat", action="store_false", default=True, dest="blat", help="include this argument if the blat filter should not be applied")
    parser.add_argument("", "--noPositionalBias", action="store_false", default=True, dest="pbias", help="include this argument if the positional bias filter should not be applied")
    parser.add_argument("", "--noRnaBlacklist", action="store_false", default=True, dest="rnaBlacklist", help="include this argument if the RNA blacklist filter should not be applied")
    parser.add_argument("", "--noSnpEff", action="store_false", default=True, dest="snpEff", help="include this argument if the snpEff annotation should not be applied (without the snpEff annotation, filtering of RNA blacklisted genes will also not be applied")
    ...
    try:
        ...
        if (args.fastaFilename != None):
            universalFastaFile = indexFasta(args.workdir, args.inputFastaFile, args.inputFastaFile + ".fai", "universal")
        ...
        if (args.dnaNormalFilename != None):
            i_dnaNormalFilename = indexBam(args.workdir, args.dnaNormalFilename, args.dnaNormalFilename + ".bai", "dnaNormal")
            
        if (args.rnaNormalFilename != None):
            i_rnaNormalFilename = indexBam(args.workdir, args.rnaNormalFilename, args.rnaNormalFilename + ".bai", "rnaNormal")
            
        if (args.dnaTumorFilename != None):
            i_dnaTumorFilename = indexBam(args.workdir, args.dnaTumorFilename, args.dnaTumorFilename + ".bai", "dnaTumor")
            
        if (args.rnaTumorFilename != None):
            i_rnaTumorFilename = indexBam(args.workdir, args.rnaTumorFilename, args.rnaTumorFilename + ".bai", "rnaTumor")
        ...
        if args.procs == 1:
            ...
            for chrom in chroms:
                ...
                execute(cmd)
                # then filter it
                cmd, out = radiaFilter(args, chrom, rawOutput, args.outputDir, args.scriptsDir)
                execute(cmd)
        else:
            chroms = get_bam_seq(i_dnaNormalFilename)
            cmds = []
            ...
            for rawOutput in rawOuts:
                ...
                cmd, filteredOut = radiaFilter(args, chrom, rawOutput, args.outputDir, args.scriptsDir, blatFastaFile)
                cmds.append(cmd)
